...
var student = [
  ["AndreusBrijbasi", "https://editor.p5js.org/abrijba1/sketches/kEEIfswWz"],
  ...
  ["ShashaChen", "https://editor.p5js.org/thewolrdisyouroyster/sketches/lIRiqoUWB"],
  ...
  ['Chien-YuanCheng', 'https://github.com/Chien-Yuan/FInal-Websocket-Connection'],
...
function setup() {
  ...
  for (i = 0; i < student.length; i++) {
    buttons[i] = new linkImages(i*200%(int(width/200)*200)+boarder, int(i*200/width)*200+70, student[i][0], student[i][1]);
  }
  textSize(45);
  fill(0, 102, 153, 51);
  text("2024 PEABODY Intro to Programming", 20, 50);
...
class linkImages{
  ...
  make(){
    image(this.button, this.posX, this.posY, 200, 200);
  }
  intersect() {
    return mouseX >= this.posX && mouseX <= this.posX+200 && mouseY >= this.posY && mouseY <= this.posY+200;
    }
  goToLink() {
